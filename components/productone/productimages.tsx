...
const ProductImages: React.FC<any> = ({ id, data }) => {
   const [active, setActive] = useState(data[0]?.pic)
         ...
         <div style={{
            ...
            <div style={{
               ...
               <div style={{ display: 'flex', justifyContent: 'center', padding: '0 0 20px 0' }}>
                  <InnerImageZoom src={`${process.env.STATIC_URL}/cards/${id}/images/size_800/${active}`} zoomScale={2} hasSpacer={true} zoomType={'click'} hideHint={true} />
               </div>
               <Swiper
               ...
               >
                  {
                     data.map((item: any) => (
                        <SwiperSlide key={item.id}
                           className={active == item.pic ? 'selected' : ''}
                           onMouseEnter={onMouseEnterHandler}
                        >
                           <div className="product-images-slider-wrapper">
                              <img src={`${process.env.STATIC_URL}/cards/${id}/images/size_150/${item.pic}`} alt="product images" />
                           </div>
                        </SwiperSlide>
                     ))
                  }
