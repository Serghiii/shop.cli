...
const MainCheckout: React.FC = () => {
    ...
    useEffect(() => {
        if (cart.started && !cart.isLoading) setCart(getDataFromCart())
    }, [cart.started])
    ...
    const onSubmitHandler = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        if (Cart.length > 0) {
            axios.post('order', { details: JSON.stringify(getDetails()), odetails: Cart}).then(({data}) => {
                // clearCart(Cart)
            }).catch(error => {
                            ...
                            <div className="checkout-frame">
                                ...
                                {Cart.length > 0 &&<div className="checkout-grid-container">
                                    {Cart.map((item:ODetails)=>(
                                        ...
                                        <div key={item.id} className="checkout-grid-item checkout-grid-item-name">{item.name}</div>
                                ...
                                <div style={{ display: "flex", justifyContent: "center", marginTop: "40px", marginBottom: "-10px", width: "100%" }}>
                                    <div>
