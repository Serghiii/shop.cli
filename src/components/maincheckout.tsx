...
import { Masks } from "../common/interfaces"
...
export default function MainCheckout() {
    ...
    const checkoutSchema = yup.object().shape({
        fio: yup.string().trim()
            .required(/*translate('checkout.messages.required', locale)*/'undefined'),
    ...
    const onSubmitHandler = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        if (Cart.length > 0) {
            await axiosService.post('order', { details: JSON.stringify(getDetails()), odetails: Cart} as Order).then(({data:Order}) => {
                // clearCart(Cart)
            }).catch(error => {
                setError(/*translate('checkout.messages.error', locale)*/'undefined')
            })
        }
    }
                        ...
                        <form className="dialog-form" onSubmit={onSubmitHandler}>
                            <div className="checkout-frame">
                                ...
                                <div className="form-row">
                                    <label htmlFor="fio" className="form-label" style={{ display: '-webkit-box', WebkitBoxOrient: 'vertical', textOverflow: 'ellipsis', WebkitLineClamp: 1, overflow: 'hidden', whiteSpace: 'normal' }}>{'checkout.fio undefined'}</label>
                                    <input
                                        {...register("fio")}
                                        id="fio"
                                ...
                                <div className="form-row">
                                    <label htmlFor="city" className="form-label" style={{ display: '-webkit-box', WebkitBoxOrient: 'vertical', textOverflow: 'ellipsis', WebkitLineClamp: 1, overflow: 'hidden', whiteSpace: 'normal' }}>{/*translate('checkout.city', locale)*/'undefined'}</label>
                                    <input
                                        {...register("city")}
                                        id="city"
                                        ...
                                        <Controller
                                            ...
                                            render={({field}) => (
                                                <RadioGroup
                                                    {...field}
                                                >
                                                    <FormControlLabel sx={{ mt: -1, mb: -1 }} value={posts[0]} control={<Radio color="primary" />} label={/*translate('checkout.shipping.new_post.title', locale)*/'undefined'} />
                                                    <FormControlLabel sx={{ mt: -1, mb: -1 }} value={posts[1]} control={<Radio color="primary" />} label={/*translate('checkout.shipping.ukr_post.title', locale)*/'undefined'} />
                                                </RadioGroup>
                                            )}
                                        />
                                        ...
                                        <Controller
                                            ...
                                            render={({field}) => (
                                                <RadioGroup
                                                    {...field}
                                                >
                                                    <FormControlLabel sx={{ mt: -1, mb: -1 }} value={/*translate('checkout.payment.card', locale)*/'undefined'} control={<Radio color="primary" />} label={/*translate('checkout.payment.card', locale)*/'undefined'} />
                                                    <FormControlLabel sx={{ mt: -1, mb: -1 }} value={/*translate('checkout.payment.cash', locale)*/'undefined'} control={<Radio color="primary" />} label={/*translate('checkout.payment.cash', locale)*/'undefined'} />
                                                    <FormControlLabel sx={{ mt: -1, mb: -1 }} value={/*translate('checkout.payment.cashless', locale)*/'undefined'} control={<Radio color="primary" />} label={/*translate('checkout.payment.cashless', locale)*/'undefined'} />
                                                </RadioGroup>
                                            )}
                                        />
                                    </FormControl>
