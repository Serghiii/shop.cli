...
const PasswordPanel: React.FC = () => {
   ...
   const validationSchema = yup.object().shape({
      password: yup.string().required(d.auth.messages.required)
         .min(6, d.auth.messages.password),
      cpassword: yup.string().required(d.auth.messages.required)
         .min(6, d.auth.messages.password)
   ...
   return (
      <form className="dialog-form-simple" onSubmit={handleSubmit(onSubmitHandle)}>
         <div className="form-row-simple">
            <label htmlFor="Password" className="form-label-simple">{d.profile.tabs.panels.newpassword}</label>
         ...
         <div className="form-row-simple">
            <label htmlFor="confirmPassword" className="form-label-simple">{d.profile.tabs.panels.reppassword}</label>
