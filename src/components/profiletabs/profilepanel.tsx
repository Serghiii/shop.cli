...
import { useTranslation } from "../../hooks/translatation";
...
const ProfilePanel: React.FC<any> = (props) => {
   ...
   const validationSchema = yup.object().shape({
      name: yup.string()
         .required(t('auth.messages.required'))
         .min(2, t('auth.messages.login'))
         .matches(/^[а-яА-ЯіІёЁ\s]+$/, t('auth.messages.login_cyr')),
      gender: yup.string()
         .required(t('auth.messages.gender'))
         .oneOf(['1', '2'], t('auth.messages.gender')),
      phone: yup.string()
         .matches(/^\+38\s[0-9,\s]+$/, t('auth.messages.required'))
         .matches(/^\+38\s\d{3}\s\d{3}\s\d{2}\s\d{2}$/, t('auth.messages.phone')),
   })
      ...
      <>
         <form className="dialog-form-simple" onSubmit={handleSubmit(onSubmitHandler)}>
            <div className="form-row-simple">
               <label htmlFor="name" className="form-label-simple">{t('profile.tabs.panels.pib')}</label>
            ...
            <div className="form-row-simple">
               <label className="form-label-simple">{t('profile.tabs.panels.gender.name')}</label>
               <Controller
                  control={control}
                  name='gender'
                  render={({field}) => (
                     <RadioGroup
                        ...
                        aria-label='gender'
                        >
                        <FormControlLabel value="1" disabled={props.gender in ['1', '2']} control={<Radio color="primary" />} label={t('profile.tabs.panels.gender.male')} />
                        <FormControlLabel value="2" disabled={props.gender in ['1', '2']} control={<Radio color="primary" />} label={t('profile.tabs.panels.gender.famale')} />
                     </RadioGroup>
                  )}
               />
            </div>
            ...
            <div className="form-row-simple">
               <label htmlFor="phone" className="form-label-simple">{t('profile.tabs.panels.phone')}</label>
                  <Controller
                     control={control}
                     name='phone'
            ...
            <div className="form-row-simple">
               <label htmlFor="email" className="form-label-simple">{t('profile.tabs.panels.email')}</label>
            ...
            <div className="form-row-simple">
               ...
               <div className="custom-button-simple-wrapper">
                  <button className="custom-button-simple">{t('profile.tabs.panels.save')}</button>
               </div>
            </div>
         </form>
      </>
