...
export const GetCartAction = createAsyncThunk('cart/GetCartAction', async (action: CartState, { rejectWithValue }) => {
	if (!action.cart) return action.cart
	try {
		if (!action.started) {
			if (action.cart.length > 0) {
				const data = await (await axiosService.post('products/cart', getIDs(action.cart))).data
				if (data.length > 0) {
					return data.map((item1: Cart) => ({
						...item1,
						iamount: action.cart.find((item2: LsCart) => item2.id === item1.id)?.amount
					}))
				}
			}
			return []
